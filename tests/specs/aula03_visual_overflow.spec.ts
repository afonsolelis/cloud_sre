...
/**
 * Visual validation of Aula 03 slide deck.
...
const SLIDE_FILE = path.resolve(
  __dirname,
  '../../aulas/aula_03_fontes_de_dados_bancos_relacionais_e_nosql/slides/slide_aula_03_fontes_de_dados_bancos_relacionais_e_nosql.html'
);

const SCREENSHOTS_DIR = path.resolve(__dirname, '../../test-results/aula03-slides');

test.describe('Aula 03 - visual overflow per slide', () => {
  test.beforeAll(() => {
    ...
    test(`every slide fits within ${vp.name}`, async ({ page }) => {
      ...
      for (let i = 0; i < totalSlides; i++) {
        ...
        const hasMermaid = await page.evaluate(() => {
          ...
          return !!active && !!active.querySelector('pre.mermaid, .a03-mermaid svg');
        });
