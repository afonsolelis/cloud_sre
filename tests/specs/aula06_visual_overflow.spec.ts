...
/**
 * Visual validation of Aula 06 slide deck.
...
const SLIDE_FILE = path.resolve(
  __dirname,
  '../../aulas/aula_06_data_warehousing_na_nuvem_de_alta_performance/slides/slide_aula_06_data_warehousing_na_nuvem_de_alta_performance.html'
);

const SCREENSHOTS_DIR = path.resolve(__dirname, '../../test-results/aula06-slides');

test.describe('Aula 06 - visual overflow per slide', () => {
  test.beforeAll(() => {
    ...
    test(`every slide fits within ${vp.name}`, async ({ page }) => {
      ...
      for (let i = 0; i < totalSlides; i++) {
        ...
        const hasMermaid = await page.evaluate(() => {
          ...
          return !!active && !!active.querySelector('pre.mermaid, .a06-mermaid svg');
        });
