...
/**
 * Visual validation of Aula 04 material.
...
const MATERIAL_FILE = path.resolve(
  __dirname,
  '../../aulas/aula_04_ingestao_e_processamento_near_real_time_streaming/material/material_aula_04_ingestao_e_processamento_near_real_time_streaming.html'
);

const SCREENSHOTS_DIR = path.resolve(__dirname, '../../test-results/aula04-material');

test.describe('Aula 04 - material visual validation', () => {
  test.beforeAll(() => {
  ...
  for (const vp of VIEWPORTS) {
    test(`material renders cleanly at ${vp.name}`, async ({ page }) => {
      ...
      await page.waitForFunction(
        () => {
          const wrap = document.querySelector('.m04-mermaid');
      ...
      // 2. Overflow horizontal por elemento (codeblocks, tabelas, .m04-mermaid)
      const hOverflows = await page.evaluate(() => {
        const offenders: Array<{ tag: string; cls: string; rectRight: number; vw: number }> = [];
        const vw = document.documentElement.clientWidth;
        const candidates = document.querySelectorAll('pre, table, .m04-mermaid, .m04-callout');
      ...
      // Screenshot da regiao do diagrama Mermaid
      const mermaidEl = await page.locator('.m04-mermaid').first();
